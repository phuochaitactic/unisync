...
export class Service {
  ...
  convertToAppointments(dataReg_Result: KetquaDK[]): Appointment[] {
    const convertedAppointments: Appointment[] = dataReg_Result.map((item) => {
      const startDate = new Date(item.ngayBd);

      // Thêm 1 giờ vào endDate
    ...
    console.log("Converted Appointments:", convertedAppointments); // Log the converted appointments
