...
@Component({
  selector: "app-quanlyvbtl",
  templateUrl: "./quanlyvbtl.component.html",
  styleUrls: ["./quanlyvbtl.component.css"],
})
export class QuanlyvbtlComponent {
  ...
  deleteSelectedRows(): void {
    ...
    if (selectedRows.length > 0) {
      selectedRows.forEach((index) => {
        ...
        if (selectedLop) {
          this.vanbanService.deleteVB(selectedLop.idvanBan).subscribe(() => {
            this.quanLyCosoData = this.quanLyCosoData.filter(
              (lop) => lop.idvanBan !== selectedLop.idvanBan
            );
  ...
  onFileSelected(event: any) {
    ...
    if (fileSizeInMB > maxSizeInMB) {
      this.toast.warning("Kích thước tệp quá lớn. Vui lòng chọn tệp dưới 1MB.");
      return;
    }
    ...
    if (this.editingLop) {
      this.vanbanService.putEditVB(this.editingLop).subscribe((data) => {
        const index = this.quanLyCosoData.findIndex(
          (k) => k.idvanBan === this.editingLop.idvanBan
        );
        if (index !== -1) {
  ...
  onDelete(idvanBan: any) {
    this.selectedLop = idvanBan;
    this.ondelete = true;
  }
  closeDelete() {
    this.ondelete = false;
  ...
  deleteVbtl() {
    if (this.selectedLop) {
      this.vanbanService.deleteVB(this.selectedLop).subscribe(() => {
        this.quanLyCosoData = this.quanLyCosoData.filter(
          (lop) => lop.idvanBan !== this.selectedLop
        );
